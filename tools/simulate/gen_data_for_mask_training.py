...
def main():
    ...
    object_poses = get_ring_object_poses(
        0.9, 1.5, 10, 70, ndist=30, nelev=20, nazim=60, trans_noise=0.05
    )
    ...
    joint_limits = [
        pymp_planner.robot.joint_limits[0][:dof],
        pymp_planner.robot.joint_limits[1][:dof],
    ]
    random_qpos = np.random.uniform(
        joint_limits[0], joint_limits[1], [cfg.random_qpos_number, dof]
    )
    ...
    for camera_pose, qpose in tqdm.tqdm(zip(camera_poses, qposes), total=N):
        ...
        rendered_mask = render_api.nvdiffrast_render_xarm_api(
            urdf_path, np.linalg.inv(camera_pose), qpose, height, width, K
        )
