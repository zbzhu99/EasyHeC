...
def main():
    ...
    parser.add_argument("--ckpt_dir", default="models/xarm6/example_xarm6_offline")
    parser.add_argument(
        "--data_dir",
        default="data/xarm6_offline/example",
        help="data dir to validate on",
    )
    ...
    vis3d = Vis3D(xyz_pattern=("x", "y", "z"), out_folder="dbg", sequence="validate")
    for i in tqdm.trange(len(rgb_paths)):
        ...
        rendered_mask = render_api.nvdiffrast_render_xarm_api(
            "assets/xarm6_with_gripper.urdf", Tc_c2b, qpos, H, W, K
        )
        tmp = plt_utils.vis_mask(
            rgb.copy(), rendered_mask.astype(np.uint8), [255, 0, 0]
        )
