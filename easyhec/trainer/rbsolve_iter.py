...
class RBSolverIterTrainer(BaseTrainer):
    ...
    def train(self, epoch):
        ...
        metric_ams = {}
        bar = (
            tqdm.tqdm(self.train_dl, leave=False)
            if is_main_process() and len(self.train_dl) > 1
            else self.train_dl
        )
        begin = time.time()
        for batchid, batch in enumerate(bar):
            ...
            if is_main_process():
                ...
                if self.global_steps % 200 == 0:
                    self.image_grid_on_tb_writer(
                        output["rendered_masks"],
                        self.tb_writer,
                        "train/rendered_masks",
                        self.global_steps,
                    )
                    self.image_grid_on_tb_writer(
                        output["ref_masks"],
    ...
    def image_grid_on_tb_writer(self, images, tb_writer, tag, global_step):
        ...
        tb_writer.add_image(
            tag, np.array(Image.open(img_buf))[:, :, :3].transpose(2, 0, 1), global_step
        )
        plt.close("all")
    ...
    def capture_data(self):
        ...
        np.savetxt(
            osp.join(outdir, "Tc_c2b.txt"), np.eye(4)
        )  # fake ground-truth Tc_c2b
        ...
        qpose = self.qposes[-1]
        vis3d.add_xarm(qpose.tolist()[:6] + [0, 0])
        arm = self.arm
            ...
            if len(self.qposes) == 1:
                ...
                if not self.cfg.model.rbsolver_iter.use_realarm.speed_control:
                    ...
                    if "PYCHARM_HOSTED" not in os.environ:
                        input(
                            "Please visualize the next joint pose in Wis3D and press Enter to drive the robot..."
                        )
                    for ti, target_qpos in enumerate(tqdm.tqdm(plan_qposes)):
                        ...
                        arm.vc_set_joint_velocity(
                            qvel_cliped, is_radian=True, is_sync=True, duration=timestep
                        )
                        time.sleep(timestep)
                    arm.set_mode(0)
                    time.sleep(1)
    ...
    def rebuild(self):
        ...
        self.scheduler = make_lr_scheduler(
            self.cfg, self.optimizer, self.cfg.solver.num_epochs * len(self.train_dl)
        )
    ...
    def initialize_Tc_c2b(self):
        if self.cfg.use_xarm is True:
            cmd = (
                f"cd {osp.abspath('.')}/third_party/pvnet && "
                f"{sys.executable} run_demo_xarm7.py -c configs/xarm7/10k.yaml "
                "demo_dir ../../data/xarm6/example"
