...
def make_lr_scheduler(cfg, optimizer, max_iter):
    if cfg.solver.scheduler == "WarmupMultiStepLR":
        ...
        return OneCycleScheduler(optimizer, cfg.solver.max_lr, max_iter)
        ...
        return ExponentialScheduler(
            optimizer, cfg.solver.gamma, cfg.solver.lrate_decay, cfg=cfg
        )
    elif cfg.solver.scheduler == "WarmupExponentialScheduler":
        return WarmupExponentialScheduler(
            optimizer, cfg.solver.warmup_iters, cfg.solver.gamma, cfg.solver.lrate_decay
        )
    elif cfg.solver.scheduler == "ExponentialStep":
        scheduler = LambdaLR(
            optimizer,
            ExponentialSchedulerLambda(
                total_steps=cfg.solver.num_iters, min_factor=cfg.solver.min_factor
            ),
        )
        return scheduler
